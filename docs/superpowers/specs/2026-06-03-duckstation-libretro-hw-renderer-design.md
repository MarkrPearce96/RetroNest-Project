...
- **Save-state save/load under the HW renderer: verified working, after fixing a resume crash it exposed.** Save-and-exit then resume initially crashed (`SIGILL`). Root cause was a **host** bug, not the core: the libretro `runLoop` (`core_runtime.cpp`) left autoreleased Metal/ObjC objects on the worker QThread's lifetime autorelease pool, then `CoreLoader::close()` `dlclose`d the core dylib; the pool drained only at thread exit — after the dylib was unmapped — so releasing an object whose class/dealloc lived in the unloaded dylib jumped to unmapped memory. The HW Metal renderer triggered it (the software path created no such per-frame Metal objects). Fixed by draining autorelease pools per-frame and before `dlclose` (RetroNest commit `c1f7281`, new `autorelease_scope.h`). Resume confirmed clean afterward. Diagnosed via the crash report (`duckstation_libretro.dylib` absent from images, faulting PC unmapped, inside `objc_autoreleasePoolPop` during `_pthread_exit`); the bug was masked by `NSZombie` (classic UAF signature).
