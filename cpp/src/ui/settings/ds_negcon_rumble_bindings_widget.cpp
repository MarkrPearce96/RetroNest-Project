...
DSNegconRumbleBindingsWidget::DSNegconRumbleBindingsWidget(SdlInputManager* inputManager,
    ...
    : BindingsWidgetBase(inputManager, appController, emuId, port, parent)
{
    ...
    auto* analogLbl = makeLabel(canvas, "Analog");
    auto* analog    = new BindBtn(canvas);
    ...
    setupBtn(start, "Start"); setupBtn(analog, "Analog");
    ...
    startLbl->setObjectName("startLbl"); analogLbl->setObjectName("analogLbl");
...
void DSNegconRumbleBindingsWidget::relayout() {
    int W = width();
    int H = height();
    Q_UNUSED(H);
    ...
    // ── CENTER: Start + Analog ──
    if (auto* l = LBL("startLbl")) l->move(cx - btnSmW - 10, kTy + 100);
    m_bindingButtons["Start"]->setGeometry(cx - btnSmW - 5, kTy + 120, btnSmW, kBtnH);

    if (auto* l = LBL("analogLbl")) l->move(cx + 15, kTy + 100);
    m_bindingButtons["Analog"]->setGeometry(cx + 5, kTy + 120, btnSmW, kBtnH);
