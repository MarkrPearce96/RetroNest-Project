...
DSJogconBindingsWidget::DSJogconBindingsWidget(SdlInputManager* inputManager,
    ...
    : BindingsWidgetBase(inputManager, appController, emuId, port, parent)
{
    ...
    // Mode button (center, below image)
    auto* modeLbl  = makeLabel(canvas, "Mode");
    auto* mode     = new BindBtn(canvas);

    // Steering Left, Steering Right
    auto* slLbl    = makeLabel(canvas, "Steering Left");
    auto* sl       = new BindBtn(canvas);
    auto* srLbl    = makeLabel(canvas, "Steering Right");
    auto* sr       = new BindBtn(canvas);

    // Motor (single)
    auto* motorLbl = makeLabel(canvas, "Motor");
    auto* motor    = new BindBtn(canvas);
    ...
    setupBtn(mode, "Mode");
    setupBtn(sl, "Steering Left"); setupBtn(sr, "Steering Right");
    setupBtn(motor, "Motor");
    ...
    modeLbl->setObjectName("modeLbl");
    slLbl->setObjectName("slLbl"); srLbl->setObjectName("srLbl");
    motorLbl->setObjectName("motorLbl");

    loadBindings();
    relayout();
}

void DSJogconBindingsWidget::relayout() {
    int W = width();
    int H = height();
    Q_UNUSED(H);
    ...
    // ── CENTER: Mode button ──
    int modeY = kTy + 180 + imgH + 10;
    int modeW = 170;
    if (auto* l = LBL("modeLbl")) l->move(cx - modeW/2 + modeW/2 - 18, modeY);
    m_bindingButtons["Mode"]->setGeometry(cx - modeW/2, modeY + 18, modeW, kBtnH);
    ...
    if (auto* l = LBL("slLbl")) l->move(cx - steerW - steerSpacing/2 + steerW/2 - 38, steerY);
    m_bindingButtons["Steering Left"]->setGeometry(cx - steerW - steerSpacing/2, steerY + 18, steerW, kBtnH);

    if (auto* l = LBL("srLbl")) l->move(cx + steerSpacing/2 + steerW/2 - 42, steerY);
    m_bindingButtons["Steering Right"]->setGeometry(cx + steerSpacing/2, steerY + 18, steerW, kBtnH);

    // ── CENTER: Motor (single) ──
    int motorY = steerY + 60;

    if (auto* l = LBL("motorLbl")) l->move(cx - modeW/2 + modeW/2 - 18, motorY);
    m_bindingButtons["Motor"]->setGeometry(cx - modeW/2, motorY + 18, modeW, kBtnH);
}
