...
NegconBindingsWidget::NegconBindingsWidget(SdlInputManager* inputManager,
    ...
    : BindingsWidgetBase(inputManager, appController, emuId, port, parent)
{
    ...
    QPixmap pix(":/AppUI/qml/AppUI/images/controllers/Negcon.svg");
    if (!pix.isNull())
    ...
    // Bottom center: Twist, Motors
    auto* twistLLbl = makeLabel(canvas, "Twist Left");
    auto* twistL    = new BindBtn(canvas);
    auto* twistRLbl = makeLabel(canvas, "Twist Right");
    auto* twistR    = new BindBtn(canvas);
    auto* lgMotorLbl = makeLabel(canvas, "LargeMotor");
    auto* lgMotor    = new BindBtn(canvas);
    auto* smMotorLbl = makeLabel(canvas, "SmallMotor");
    ...
    setupBtn(twistL, "Twist Left"); setupBtn(twistR, "Twist Right");
    ...
    twistLLbl->setObjectName("twistLLbl"); twistRLbl->setObjectName("twistRLbl");
...
void NegconBindingsWidget::relayout() {
    int W = width();
    int H = height();
    Q_UNUSED(H);
    ...
    // ── CENTER: Twist Left, Twist Right ──
    int row3Y = kTy + 180 + imgH + 10;
    int itemW = 170;
    int spacing = 10;

    if (auto* l = LBL("twistLLbl")) l->move(cx - itemW - spacing/2 + itemW/2 - 30, row3Y);
    m_bindingButtons["Twist Left"]->setGeometry(cx - itemW - spacing/2, row3Y + 18, itemW, kBtnH);

    if (auto* l = LBL("twistRLbl")) l->move(cx + spacing/2 + itemW/2 - 35, row3Y);
    m_bindingButtons["Twist Right"]->setGeometry(cx + spacing/2, row3Y + 18, itemW, kBtnH);

    // ── CENTER: LargeMotor, SmallMotor ──
    int row4Y = row3Y + 60;

    if (auto* l = LBL("lgMotorLbl")) l->move(cx - itemW - spacing/2 + itemW/2 - 35, row4Y);
    m_bindingButtons["LargeMotor"]->setGeometry(cx - itemW - spacing/2, row4Y + 18, itemW, kBtnH);

    if (auto* l = LBL("smMotorLbl")) l->move(cx + spacing/2 + itemW/2 - 35, row4Y);
    m_bindingButtons["SmallMotor"]->setGeometry(cx + spacing/2, row4Y + 18, itemW, kBtnH);
}
