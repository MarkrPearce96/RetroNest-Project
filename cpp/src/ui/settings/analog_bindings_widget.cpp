...
AnalogBindingsWidget::AnalogBindingsWidget(SdlInputManager* inputManager,
    ...
    : BindingsWidgetBase(inputManager, appController, emuId, port, parent)
{
    ...
    QPixmap pix(":/AppUI/qml/AppUI/images/controllers/analog_controller.svg");
    if (!pix.isNull())
    ...
    // Bottom center buttons: L3, Analog, R3
    ...
    l3Lbl->setObjectName("l3Lbl"); r3Lbl->setObjectName("r3Lbl");
    anaLbl->setObjectName("anaLbl");
...
void AnalogBindingsWidget::relayout() {
    int W = width();
    int H = height();
    Q_UNUSED(H);
    ...
    // ── CENTER: L3, Analog, R3 (3 items, no Pressure Modifier) ──
    int row3W = 170;
    ...
    if (auto* l = LBL("anaLbl")) l->move(cx - 22, row3Y);
    m_bindingButtons["Analog"]->setGeometry(row3Start + row3W + row3Spacing, row3Y + 18, row3W, kBtnH);
    ...
    int motorW = 170;
    int motorSpacing = 20;
    if (auto* l = LBL("lgMotorLbl")) l->move(cx - motorW - motorSpacing/2 + 10, motorY);
    m_bindingButtons["LargeMotor"]->setGeometry(cx - motorW - motorSpacing/2, motorY + 18, motorW, kBtnH);

    if (auto* l = LBL("smMotorLbl")) l->move(cx + motorSpacing/2 + 10, motorY);
    m_bindingButtons["SmallMotor"]->setGeometry(cx + motorSpacing/2, motorY + 18, motorW, kBtnH);
}
