...
AnalogJoystickBindingsWidget::AnalogJoystickBindingsWidget(SdlInputManager* inputManager,
    ...
    : BindingsWidgetBase(inputManager, appController, emuId, port, parent)
{
    ...
    QPixmap pix(":/AppUI/qml/AppUI/images/controllers/analog_joystick.svg");
    if (!pix.isNull())
    ...
    // Bottom center buttons: L3, Mode, R3
...
void AnalogJoystickBindingsWidget::relayout() {
    int W = width();
    int H = height();
    Q_UNUSED(H);
    ...
    // ── CENTER: L3, Mode, R3 (3 items, no motors) ──
    int row3W = 170;
    ...
    if (auto* l = LBL("modeLbl")) l->move(cx - 18, row3Y);
    m_bindingButtons["Mode"]->setGeometry(row3Start + row3W + row3Spacing, row3Y + 18, row3W, kBtnH);
