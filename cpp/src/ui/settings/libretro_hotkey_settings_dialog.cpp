#include "libretro_hotkey_settings_dialog.h"
...
LibretroHotkeySettingsDialog::LibretroHotkeySettingsDialog(SdlInputManager* inputManager,
                                                            AppController* appController,
                                                            QWidget* parent)
    : EmulatorSettingsDialogBase(appController,
                                  libretro_hotkeys::kSentinelEmuId,
                                  parent),
      m_inputManager(inputManager) {
    setupChrome(QStringLiteral("Libretro Hotkeys"),
                QSize(900, 720),
                SettingsDialogTheme::windowBg());

    m_page = new GenericHotkeyPage(inputManager, appController,
                                    libretro_hotkeys::kSentinelEmuId, this);
    connect(m_page, &GenericHotkeyPage::bindingFocused,
            this, &LibretroHotkeySettingsDialog::onBindingFocused);
    ...
    connect(restore, &QPushButton::clicked, this,
            &LibretroHotkeySettingsDialog::onRestoreDefaultsClicked);
...
void LibretroHotkeySettingsDialog::onBindingFocused(HotkeyDef def, QString currentDisplay) {
    if (m_descBar) m_descBar->setHotkey(def, currentDisplay);
}

void LibretroHotkeySettingsDialog::onRestoreDefaultsClicked() {
    if (m_page) m_page->restoreDefaults();
}

void LibretroHotkeySettingsDialog::keyPressEvent(QKeyEvent* e) {
    if (!m_page) {
...
void LibretroHotkeySettingsDialog::showEvent(QShowEvent* e) {
