...
QVector<SettingDef> PCSX2Adapter::settingsSchema() const {
    ...
    // Hardware Fixes + Upscaling Fixes panes deliberately not exposed —
    // upstream PCSX2 gates both behind the "Manual Hardware Renderer Fixes"
    // (UserHacks) toggle which itself is only visible in per-game / dev-build
    // mode. In a default standalone install they're hidden; users tweak
    // these per-game via PCSX2's compatibility database. See
    // pcsx2-schema-alignment.md for the full UserHacks_* key list and the
    ...
    // Advanced + Debug panes intentionally not surfaced — see
    // pcsx2-schema-alignment.md.
