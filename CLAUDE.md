...
- **`GenericMultiCardPicker`** — a card + pill 2-D grid with a preview image and (optional) trailing Save button (Aspect Ratio). **`GenericRowPicker`** — the preview-less sibling: one full-width row per emulator (badge + name + pills), used where a preview can't convey the choice (Resolution). Both render their pills via the shared **`PillRow`** and auto-save on select through the quick-settings config_service methods.
